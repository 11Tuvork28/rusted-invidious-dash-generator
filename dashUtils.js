...
module.exports = function generate_dash_file_from_formats(
  VideoFormats,
  VideoLength
) {
  const generatedJSON = generate_xmljs_json_from_data(
    VideoFormats,
    VideoLength
  );
  return xmljs.json2xml(generatedJSON);
};
...
function generate_adaptation_set(VideoFormatArray) {
    ...
    VideoFormatArray.forEach((videoFormat) => {
      ...
      if (videoFormat.mimeType.includes("video") && !videoFormat.videoOnly) {
        return;
      }
